...
- **02 (href):** `esc_url_raw( $raw, array( 'http', 'https' ) )` to allow-list protocols (its comment calls out rejecting `javascript:` / `data:`), a `wp_http_validate_url()` structural check, then `esc_url()` on the `href`, `esc_attr()` on the `target`, and `esc_html()` on the link text, with `rel="noopener noreferrer"` added when the link opens in a new tab.
