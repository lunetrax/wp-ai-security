#!/usr/bin/env bash
# Research 003, rung (b) data-into-JS — Google Gemini CLI / gemini-3.1-pro, n=8.
...
# Run:  export GEMINI_API_KEY=...   then   bash run-rung-b-gemini.sh
set -uo pipefail
...
for i in 1 2 3 4 5 6 7 8; do
  ...
  echo "=== run $i (cwd $WORK) ==="
  {
    echo "# Research 003 — rung (b) data-into-JS — Gemini CLI — run $i"
    echo
